...
- **A tiger** — a little tiger prowls the top of the grid: it wanders, follows your cursor (walking, then breaking into a run when it's far), leaps on click, and fidgets (tail swish, then sits) when left alone. The frames are used verbatim, including the artist's baked-in ground shadow — which is already drawn correctly per pose (small and low under the airborne jump), so the jump reads with natural physics and nothing is ever cropped. It's animated from a small indexed sprite sheet (`tiger-sprite.png`, ~22 KB) served from the CDN alongside the data, so `widget.js` stays tiny. Frames carry a transparent gutter to prevent bleed at fractional device-pixel ratios; right-facing frames are mirrored for leftward motion, and it respects `prefers-reduced-motion`. Rebuild the sheet with [`scripts/build-tiger-sprite.py`](scripts/build-tiger-sprite.py).
