...
- **A tiger** — a little tiger prowls the top of the grid: it wanders, follows your cursor (walking, then breaking into a run when it's far), leaps on click, and fidgets (tail swish, then sits) when left alone. It casts a soft ground shadow that stays on the baseline and shrinks/fades as it jumps (the baked-in shadow is stripped from the frames so the physics reads naturally). It's animated from a small indexed sprite sheet (`tiger-sprite.png`, ~22 KB) served from the CDN alongside the data, so `widget.js` stays tiny. Right-facing frames are mirrored for leftward motion, and it respects `prefers-reduced-motion`. Rebuild the sheet with [`scripts/build-tiger-sprite.py`](scripts/build-tiger-sprite.py).
