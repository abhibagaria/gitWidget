...
CELL_H = 64                # sheet cell height in px
...
def build(frames, out_path):
    ...
    for i, im in enumerate(frames):
        sheet.paste(im.resize((cell_w, CELL_H), Image.LANCZOS), (i * cell_w, 0))
    ...
    return cell_w, n, os.path.getsize(out_path)
...
if __name__ == "__main__":
    ...
    cell_w, n, size = build(frames, out)
    print("wrote", os.path.abspath(out))
    print(f"cell {cell_w}x{CELL_H}px · {n} frames · {size/1024:.1f} KB")
